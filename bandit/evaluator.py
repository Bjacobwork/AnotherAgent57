...
def episode(dtype, env, env_name, obs_shape, num_actions, hidden_depth, max_episode_length, N, j, beta, motivation,
            agent, reward_scale, evaluate_modified_reward, reward_cutout_length=500):
    ...
    for s in range(max_episode_length):
        ...
        score += total_reward if evaluate_modified_reward else reward_e
...
def evaluate(params, device, checkpoint_dir):
    ...
    with tf.device(device):
        ...
        evaluate_modified_reward = params['Misc']['evaluate_modified_reward']
        ...
        while True:
            ...
            for i in range(evaluation_iterations):
                ...
                total_reward, score = episode(dtype, env, env_name, obs_shape, num_actions, hidden_depth,
                                              max_episode_length,
                                              N, j, beta, motivation, agent, reward_scale, evaluate_modified_reward)
