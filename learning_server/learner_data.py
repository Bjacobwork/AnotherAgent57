...
class LearnerData:
    ...
    def __init__(self, params, address=None):
        ...
        memory_size = batch_size * (dtype_size * (
                    4 * trace_length + hidden_size + 1 + obs_size * trace_length) + 2 * element_size + 1 + trace_length) + dtype_size + 2
        if address:
        ...
        self.observations = np.ndarray((trace_length, batch_size, obs_shape[1], obs_shape[2], obs_shape[3]),
                                       dtype=dtype, buffer=self.shared_mem.buf[start:end])
        start = end
        ...
        self.prev_extrinsic_rewards = np.ndarray((trace_length, batch_size, 1), dtype=dtype,
                                                 buffer=self.shared_mem.buf[start:end])
        start = end
        end += dtype_size * batch_size * trace_length
        self.prev_intrinsic_rewards = np.ndarray((trace_length, batch_size, 1), dtype=dtype,
                                                 buffer=self.shared_mem.buf[start:end])
        start = end
        end += dtype_size * batch_size * trace_length
