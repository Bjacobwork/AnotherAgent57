...
def process_batch(data, dtype, num_actions, N, trace_length, replay_period, retrace_lambda, target, online, im_model,
                  training_splits, break_early, eta):
    ...
    q_values = []
    for i, x, a, re, ri in zip(range(trace_length + 1), observations, one_hot_actions, prev_extrinsic_rewards,
                               prev_intrinsic_rewards):
    ...
    retrace_targets = [tf.expand_dims(dqn.h(q_values[:, replay_period] + (tf.reduce_sum(
        gamma * reduced_product(c[:, 1:], dtype) * temporal_difference[:, replay_period:], axis=-1))), 0)]
    for i, q in zip(range(1, trace_length - replay_period), tf.transpose(q_values[:, replay_period + 1:], [1, 0])):
        retrace_targets.append(tf.expand_dims(dqn.h(q + (tf.reduce_sum(
            gamma[:, :-i] * reduced_product(c[:, i + 1:], dtype) * temporal_difference[:, replay_period + i:],
            axis=-1))), 0))

    retrace_targets = tf.concat(retrace_targets, axis=0)
    splits = training_splits

    for x_, a_, re_, ri_, action_taken, retrace_target in zip(
            tf.split(observations[replay_period:-1], num_or_size_splits=splits, axis=0),
        ...
        online.extrinsic_model.optimizer.apply_gradients(
            zip(grads[:extrinsic_variable_len], trainable_variables[:extrinsic_variable_len]))
        online.intrinsic_model.optimizer.apply_gradients(
            zip(grads[extrinsic_variable_len:], trainable_variables[extrinsic_variable_len:]))
        if break_early:
            break
    ...
    if data.init_step_count[0] > 0:
        ...
        oeh, oec, oih, oic = tf.split(tf.zeros((data.init_step_count[0], oeh.shape[1] * 4), dtype=dtype),
                                      num_or_size_splits=4, axis=-1)
        q_probs = tf.nn.softmax(qt, axis=-1)
        q_probs = tf.reduce_sum(tf.transpose(one_hot_actions, [1, 0, 2])[:data.init_step_count[0],
                                1:replay_period + 1] * q_probs[:data.init_step_count[0],:replay_period],axis=-1)
        ...
        retrace_targets = [tf.expand_dims(dqn.h(q_values[:data.init_step_count[0], 0] + (tf.reduce_sum(
            gamma * reduced_product(c[:, 1:], dtype) * temporal_difference[:data.init_step_count[0], :replay_period],
            axis=-1))), 0)]
        for i, q in zip(range(1, replay_period), tf.transpose(q_values[:data.init_step_count[0], 1:], [1, 0])):
            retrace_targets.append(tf.expand_dims(dqn.h(q + (tf.reduce_sum(
                gamma[:, :replay_period - i] * reduced_product(c[:, i + 1:replay_period],
                dtype) * temporal_difference[:data.init_step_count[0],i:replay_period],axis=-1))), 0))

        retrace_targets = tf.concat(retrace_targets, axis=0)
        splits = training_splits * 2
        s = 0
        for x_, a_, re_, ri_, action_taken, retrace_target in zip(
                tf.split(observations[:replay_period, :data.init_step_count[0]], num_or_size_splits=splits, axis=0),
                tf.split(one_hot_actions[:replay_period, :data.init_step_count[0]], num_or_size_splits=splits, axis=0),
                tf.split(prev_extrinsic_rewards[:replay_period, :data.init_step_count[0]], num_or_size_splits=splits,
                         axis=0),
                tf.split(prev_intrinsic_rewards[:replay_period, :data.init_step_count[0]], num_or_size_splits=splits,
                         axis=0),
                tf.split(one_hot_actions[1:1 + replay_period, :data.init_step_count[0]], num_or_size_splits=splits,
                         axis=0),
                tf.split(retrace_targets, num_or_size_splits=splits, axis=0)):
            with tf.GradientTape() as tape:
                q = []
    ...
    one_hot_actions = tf.split(
        tf.reshape(one_hot_actions, (one_hot_actions.shape[0] * one_hot_actions.shape[1], one_hot_actions.shape[2]))[
        batch_size:],
        ...
        with tf.device(device):
            ...
            while True:
                for data in batch_data:
                    ...
                    with data.lock:
                        timer = time.time()
                        process_batch(data, dtype, num_actions, N, trace_length, replay_period, retrace_lambda, target,
